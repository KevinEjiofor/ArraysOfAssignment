...
class MenstrualApp:
    ...
    def create_an_account(self):
        try:
            ...
            nickname = self.get_input("What name do you want to be called: ").strip()
    ...
    def period_status(self):
        try:
            start_menstruation_ = self.get_input("When did you start menstruation (yyyy-MM-dd): ").strip()
            end_menstruation_ = self.get_input("When did you stop your menstruation (yyyy-MM-dd): ").strip()

            start_menstruation = datetime.strptime(start_menstruation_, "%Y-%m-%d").date()
    ...
    def ovulation_period(self):
        try:
            ...
            start_menstruation_str = self.get_input("When did you start your menstruation (yyyy-MM-dd): ").strip()
            end_menstruation_str = self.get_input("When did your menstruation stop (yyyy-MM-dd): ").strip()

            start_menstruation_str = datetime.strptime(start_menstruation_str, "%Y-%m-%d").date()
            end_menstruation_str = datetime.strptime(end_menstruation_str, "%Y-%m-%d").date()

            self.cycles.set_start_menstruation(start_menstruation_str)
            self.cycles.set_end_menstruation(end_menstruation_str)
            ...
            self.display_message(
                ...
                "Pregnancy happens if a man's sperm meet and fertilise the egg. Sperm can survive in the fallopian"
                "tubes for up to 7 days after sex.")

            self.display_message(
                f"{self.user_name}, your ovulation period is from: {self.cycles.calculate_ovulation_period()}")
            self.menu_page()
